...
signed main(){
    ...
    for(pair<int, vector<int> > p: cnt){
        v.push_back({p.second.size(), p.first});
    }
    ...
    for(int i=0; i<avail; i++){
        ...
        int cur = min(want, have);

        k-=cur;

        random_shuffle(all(cnt[v[i].second]));

        for(int j=0; j<cur; j++){
            ans.push_back(cnt[v[i].second][j]);
        }
    }

    sort(all(ans));

    for(int x: ans) cout<<x+1<<nline;
